...
string labels_JSON = """{ "options": [{ "text": "gain: %roxgain%", "visible": true, "color": {"r": 1, "v": 1, "b": 1, "auto": true}, "position": { "x": 0.2956, "y": 0.99 }, "size": 15.000 },{ "text": "gain: %stamgain% amount: #stam/#stammax", "visible": true, "color": {"r": 1, "v": 1, "b": 1, "auto": true}, "position": { "x": 0.5957, "y": 0.99 }, "size": 15.000 }], "recording_variables": "#.posY;#.posX;#.posZ", "grip": false }""";
...
CTrackMania @app; // root node for interacting with maniaplanet
...
vec4 get_rgb(float hue)
{
   ...
   for (float i = 0; i < 3; ++i)  // mafs stonks
   {
...
int64 get_from_hex(const string &in hex, uint sum, int index)
{
...
string format_placeholders(string _text, CSmScriptPlayer@ sm_script)
{
   if (_text.Contains("rox"))
   {
      if (_text.Contains("roxgain"))
      {
         _text = Regex::Replace(_text, "%roxgain", "" + sm_script.AmmoGain * 100);
         _text = Regex::Replace(_text, "#roxgain", "" + sm_script.AmmoGain * 1.58); //  ?__?
      }
      ...
      if (_text.Contains("rox"))
      {
         ...
         {
            _text = Regex::Replace(_text, "#roxmax", "" + mode.GetPlayerAmmoMax(sm_script, weapon));

            _text = Regex::Replace(_text, "%rox", "" + mode.GetPlayerAmmo(sm_script, weapon) / (mode.GetPlayerAmmoMax(sm_script, weapon) / 100.f));
            _text = Regex::Replace(_text, "#rox", "" + mode.GetPlayerAmmo(sm_script, weapon)); //  ?__?
         }
      }
   }

   if (_text.Contains("stammax"))
   {
      _text = Regex::Replace(_text, "%stammax", "" + Math::Floor(sm_script.StaminaMax * 1000) / 10);
      _text = Regex::Replace(_text, "#stammax", "" + sm_script.StaminaMax * 3600);
   }
   if (_text.Contains("%stamgain"))
   {
      _text = Regex::Replace(_text, "%stamgain", "" + Math::Floor(sm_script.StaminaGain * 1000) / 10);
   }

   if (_text.Contains("stam")) // the two previous tokens contain this one!..
   {
      _text = Regex::Replace(_text, "%stam", "" + Math::Floor(sm_script.Stamina / sm_script.StaminaMax / 3.6f) / 10);
      _text = Regex::Replace(_text, "#stam", "" + sm_script.Stamina);
   }

   if (_text.Contains("yaw"))
   {
      _text = Regex::Replace(_text, "#.yaw", "" + Math::Floor(sm_script.AimYaw * 10) / 10);
      _text = Regex::Replace(_text, "#yaw", "" + sm_script.AimYaw);
   }

   if (_text.Contains("pitch"))
   {
      _text = Regex::Replace(_text, "#.pitch", "" + Math::Floor(sm_script.AimPitch * 10) / 10);
      _text = Regex::Replace(_text, "#pitch", "" + sm_script.AimPitch);
   }

   if (_text.Contains("speed"))
   {
      if (_text.Contains("hspeed"))
      {
         float hspeed = Math::Sqrt(sm_script.Velocity.x*sm_script.Velocity.x + sm_script.Velocity.z*sm_script.Velocity.z);
         _text = Regex::Replace(_text, "#.hspeed", "" + hspeed * 3.6f);
         _text = Regex::Replace(_text, "#hspeed", "" + Math::Floor(hspeed * 36) / 10);
      }
      if (_text.Contains("vspeed"))
      {
         _text = Regex::Replace(_text, "#.vspeed", "" + sm_script.Velocity.y * 3.6f);
         _text = Regex::Replace(_text, "#vspeed", "" + Math::Floor(sm_script.Velocity.y * 36) / 10);
      }

      if (_text.Contains("speed")) // the two previous tokens contain this one!..
      {
         _text = Regex::Replace(_text, "#.speed", "" + sm_script.Speed * 3.6f);
         _text = Regex::Replace(_text, "#speed", "" + Math::Floor(sm_script.Speed * 36) / 10);
      }
   }

   if (_text.Contains("pos/dt"))
   {
      if (_text.Contains("dhpos/dt2"))
      {
         float hspeed = Math::Sqrt(acceleration_cache.x*acceleration_cache.x + acceleration_cache.z*acceleration_cache.z);

         _text = Regex::Replace(_text, "#.dhpos/dt2", "" + hspeed);
         _text = Regex::Replace(_text, "#dhpos/dt2", "" + Math::Floor(hspeed * 36) / 10);
      }

      if (_text.Contains("dhpos/dt"))
      {
         float hspeed = Math::Sqrt(velocity_cache.x*velocity_cache.x + velocity_cache.z*velocity_cache.z);

         _text = Regex::Replace(_text, "#.dhpos/dt", "" + hspeed);
         _text = Regex::Replace(_text, "#dhpos/dt", "" + Math::Floor(hspeed * 36) / 10);
      }
      if (_text.Contains("dvpos/dt2"))
      {
         _text = Regex::Replace(_text, "#.dvpos/dt2", "" + acceleration_cache.y * 3.6f);
         _text = Regex::Replace(_text, "#dvpos/dt2", "" + Math::Floor(acceleration_cache.y * 36) / 10);
      }
      if (_text.Contains("dvpos/dt"))
      {
         _text = Regex::Replace(_text, "#.dvpos/dt", "" + velocity_cache.y * 3.6f);
         _text = Regex::Replace(_text, "#dvpos/dt", "" + Math::Floor(velocity_cache.y * 36) / 10);
      }

      if (_text.Contains("dpos/dt2"))
      {
         ...
         _text = Regex::Replace(_text, "#.dpos/dt2", "" + speed * 3.6f);
         _text = Regex::Replace(_text, "#dpos/dt2", "" + Math::Floor(speed * 36) / 10);
      }

      if (_text.Contains("dpos/dt"))
      {
         ...
         _text = Regex::Replace(_text, "#.dpos/dt", "" + speed * 3.6f);
         _text = Regex::Replace(_text, "#dpos/dt", "" + Math::Floor(speed * 36) / 10);
      }
   }



   if (_text.Contains("pos"))
   {
      if (_text.Contains("posX"))
      {
         _text = Regex::Replace(_text, "#.posX", "" + sm_script.Position.x);
         _text = Regex::Replace(_text, "#posX", "" + Math::Floor(sm_script.Position.x * 10) / 10);
      }
      if (_text.Contains("posY"))
      {
         _text = Regex::Replace(_text, "#.posY", "" + sm_script.Position.y);
         _text = Regex::Replace(_text, "#posY", "" + Math::Floor(sm_script.Position.y * 10) / 10);
      }
      if (_text.Contains("posZ"))
      {
         _text = Regex::Replace(_text, "#.posZ", "" + sm_script.Position.z);
         _text = Regex::Replace(_text, "#posZ", "" + Math::Floor(sm_script.Position.z * 10) / 10);
      }

      if (_text.Contains("#.pos"))
      {
         _text = Regex::Replace(_text, "#.pos", "<" + sm_script.Position.x + ", " + sm_script.Position.y + ", " + sm_script.Position.z + ">");
      }
      if (_text.Contains("#pos"))
      {
         _text = Regex::Replace(_text, "#pos", "<" + Math::Floor(sm_script.Position.x * 10) / 10 + ", " + Math::Floor(sm_script.Position.y * 10) / 10 + ", " + Math::Floor(sm_script.Position.z * 10) / 10 + ">");
      }
   }

   if (grip_hook !is null && _text.Contains("grip"))
   {
      ...
      if (grip_ptr != 0) {
         ...
         if (_text.Contains("gripX"))
         {
            _text = Regex::Replace(_text, "#.gripX", "" + grip.x);
            _text = Regex::Replace(_text, "#gripX", "" + Math::Floor(grip.x * 10) / 10);
         }
         if (_text.Contains("gripY"))
         {
            _text = Regex::Replace(_text, "#.gripY", "" + grip.y);
            _text = Regex::Replace(_text, "#gripY", "" + Math::Floor(grip.y * 10) / 10);
         }
         if (_text.Contains("gripZ"))
         {
            _text = Regex::Replace(_text, "#.gripZ", "" + grip.z);
            _text = Regex::Replace(_text, "#gripZ", "" + Math::Floor(grip.z * 10) / 10);
         }

         if (_text.Contains("grip"))
         {
            _text = Regex::Replace(_text, "#gripyaw", "" + Math::Atan2(grip.x, grip.z));
            _text = Regex::Replace(_text, "#.grip", "" + grip.Length());
            _text = Regex::Replace(_text, "#grip", "" + Math::Floor(grip.Length() * 10) / 10);
         }
      }
   }
   ...
   if (_text.Contains("#{"))
   {
      auto addresses_groups = Regex::SearchAll(_text, "#\\{([0-9a-fA-F]+)\\}");
      for (uint i = 0; i < addresses_groups.Length; i++)
      {
         ...
         if (address_groups.Length > 1)
         {
            ...
            _text = Regex::Replace(_text, "#\\{" + address_groups[1] + "\\}", "" + accel);
         }
      }
   }
   return _text;
